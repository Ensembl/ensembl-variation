...
package Bio::EnsEMBL::Variation::StrainSlice;
...
=head2 new
    ...
    Example     : $strainSlice = Bio::EnsEMBL::Variation::StrainSlice->new(-.... => ,
							      -strain_name => $strain_name);
    Description : Creates a new Bio::EnsEMBL::Variation::StrainSlice object that will contain a shallow copy of the
                  ...
                  reference sequence
    ReturnType  : Bio::EnsEMBL::Variation::StrainSlice
    Exceptions  : none
    Caller      : general
...
sub new {
  ...
  my $af_adaptor = $self->adaptor->db->get_db_adaptor('variation')->get_AlleleFeatureAdaptor();
  my $sample_adaptor = $self->adaptor->db->get_db_adaptor('variation')->get_SampleAdaptor;
...
sub ref_subseq {
  ...
  if ($self->adaptor) {
    my $seqAdaptor = $self->adaptor->db->get_SequenceAdaptor();
    $subseq = ${$seqAdaptor->fetch_by_Slice_start_end_strand($self, $start, $end, $strand)};
  } else {
...
=head2 remove_indels
  ...
  Description : Removes insertions and deletions from the allele features of this object
  ReturnType  : none
  Exceptions  : none
  Caller      : webteam
