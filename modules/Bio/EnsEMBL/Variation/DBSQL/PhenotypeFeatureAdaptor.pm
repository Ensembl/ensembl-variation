  ...
  my $sth = $self->dbc->prepare(qq{
    SELECT
      ...
      CONCAT_WS('; ',
        ...
        GROUP_CONCAT(IF(at.code in ('risk_allele', 'clinvar_clin_sig', 'clinvar_ref_allele'), at.code, NULL), "=", concat('', pfa.value, '') SEPARATOR '; ')
      ) AS attribute
      ...
      AND EXISTS(select value from phenotype_feature_attrib where phenotype_feature_id = pf.phenotype_feature_id && (attrib_type_id = 483 || attrib_type_id = 583))
