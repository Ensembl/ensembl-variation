...
use Bio::EnsEMBL::Variation::Pipeline::VariantQC::VariantQC qw( check_four_bases check_illegal_characters remove_ambiguous_alleles find_ambiguous_alleles);
...
ok( check_four_bases("A/T/G/G/C")   eq 1 ,              "Pipeline::VariantQC check_four_bases positive");
ok( check_four_bases("A/T/-/AA/C")  eq 0 ,              "Pipeline::VariantQC check_four_bases negative");


my $ambiguous_alleles_pos = find_ambiguous_alleles("M/T" );
ok ($ambiguous_alleles_pos->[0]        eq "M",             "Pipeline::VariantQC find_ambiguous_alleles positive");

my $ambiguous_alleles_neg = find_ambiguous_alleles("A/T");
ok (! defined $ambiguous_alleles_neg->[0],                "Pipeline::VariantQC find_ambiguous_alleles negative");


my  $illegal_characters_pos = check_illegal_characters("FISH");
ok( $illegal_characters_pos->[0] eq "FISH",          "Pipeline::VariantQC illegal_characters positive");

my  $illegal_characters_neg = check_illegal_characters("A/T");
ok( ! defined $illegal_characters_neg->[0] ,          "Pipeline::VariantQC illegal_characters negative");

my  $illegal_characters_neg2 = check_illegal_characters("M/T");
ok( ! defined $illegal_characters_neg2->[0] ,          "Pipeline::VariantQC illegal_characters negative (ambig)");


ok( remove_ambiguous_alleles("M/T") eq "[AC]/T",         "Pipeline::VariantQC remove_ambiguous_alleles positive ");

ok( remove_ambiguous_alleles("A/T") eq "A/T",            "Pipeline::VariantQC remove_ambiguous_alleles negative ");
