...
if ($chromOnly || $gene){ #if chrom or gene specified then use the most stringent
  if ($gene){
    ...
    @geneIDs = @{ $chr_slice->get_all_Genes_by_type($biotype, undef, 1)};
  ...
  while (my $slice = shift @$chroms) {
    ...
    @geneIDs = @{ $slice->get_all_Genes_by_type($biotype, undef, 1)};
    processGenes(\@geneIDs, $FHO);
  }
}
...
sub processGenes{
  ...
  foreach my $geneID (@$geneArr) {
    ...
    generateSNPs(\@parts,$geneID->external_name, $fho);
  }
}  
