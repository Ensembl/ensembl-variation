...
## my $sql = qq{select s.query_name from ssahaSNP_feature s, query_match_length_strain q where target_start > $start and substring_index(substring_index(target_name,'-',2),'-',1) = '$name' and target_end < $end and target_start < $end and s.query_name = q.query_name and q.strain_name = 'HuBB'};
 #my $call = qq{mysql -h ens-genomics1 -u ensro $database_name -e "$sql" > /lustre/work1/ensembl/dr2/data/reads_encode/$encode_region\.$individual_name};
 my $call = qq{mysql -h ens-genomics1 -u ensro $database_name -e "$sql" > $reads_names};
print $call,"\n";
system($call);
system("cp $index_name $new_index_name");
# #open IN, "</lustre/work1/ensembl/dr2/data/reads_encode/$encode_region\.$individual_name" || die "Could not open file with reads:$!\n";
# open IN, "</lustre/work1/ensembl/dr2/data/reads_encode/$encode_region\.$individual_name\_yuan" || die "Could not open file with reads:$!\n";
# #open OUT , ">/lustre/work1/ensembl/dr2/data/reads_encode/$encode_region\.$individual_name.out" || die "Could not open output file $!\n";
# open OUT , ">/lustre/work1/ensembl/dr2/data/reads_encode/$encode_region\.$individual_name\_yuan.out" || die "Could not open output file $!\n";
