  ...
  foreach my $p (@{$publications}){
    ...
    if(defined $variation_ids){
      foreach my $var_id (@{$variation_ids}){
        ...
        my $update_evidence_sth = $dba->dbc->prepare(qq[ update variation set evidence_attribs = TRIM(BOTH ',' FROM REPLACE(CONCAT(',', evidence_attribs, ','), ',$attrib,', ',')) WHERE variation_id = $var_id ]);
        my $update_evidence_vf_sth = $dba->dbc->prepare(qq[ update variation_feature set evidence_attribs = TRIM(BOTH ',' FROM REPLACE(CONCAT(',', evidence_attribs, ','), ',$attrib,', ',')) WHERE variation_id = $var_id ]);
