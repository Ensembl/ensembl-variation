...
my $sql1 = "DELETE FROM phenotype WHERE phenotype_id NOT IN (SELECT phenotype_id FROM phenotype_feature)";
my $sql2 = "DELETE FROM phenotype_ontology_accession WHERE phenotype_id NOT IN (SELECT phenotype_id FROM phenotype)";
...
foreach my $hostname (@hostnames) {
  ...
  while (my ($dbname) = $sth_h->fetchrow_array) {
    ...
    # Cleanup databases
    get_connection_and_query($dbname, $hostname, $sql1);
    get_connection_and_query($dbname, $hostname, $sql2);
    print STDERR "  - Cleanup: done\n";
    
    # Optimise phenotype related tables
    foreach my $table (@tables) {
      get_connection_and_query($dbname, $hostname, "OPTIMIZE TABLE ".$table);
    }
...
sub usage {
  ...
  print qq{
  Usage: perl post_process_all_phenotype_features.pl [OPTION]
  
  Post processing of the phenotype related tables.
