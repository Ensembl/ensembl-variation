...
sub variation_feature{
    ...
    dumpSQL($self->{'dbSNP'}, qq{SELECT t1.snp_id, 
				 IF(t2.contig_label = 'reference',t2.contig_chr,t2.contig_label), 
   ...
   debug("Loading SNPLoc data");
    ...
    create_and_load($self->{'dbVariation'}, "tmp_contig_loc_chrom", "snp_id i*", "chr *", "start i", 
		    "end i", "strand i");
#
...
#     #creating the temporary table with the genotyped variations
    ...
    $self->{'dbVariation'}->do(qq{INSERT INTO variation_feature_chrom (variation_id, seq_region_id,seq_region_start, seq_region_end, seq_region_strand,
