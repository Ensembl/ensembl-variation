...
sub insert_transcript_variation {
  my $dbhvar = shift;
  my $load_file = shift;
 ...
 my $sql = $dbhvar->prepare(qq[INSERT INTO transcript_variation (transcript_variation_id, variation_feature_id, feature_stable_id, allele_string, somatic, consequence_types, cds_start, cds_end, cdna_start, cdna_end, translation_start, translation_end, distance_to_transcript, display) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)]);
  ...
  while (<FH>) {
    ...
    my $variation_feature_id = (split)[1];
    my $feature_stable_id = (split)[2];
    my $allele_string = (split)[3];
    my $somatic = (split)[4];
    my $consequence_types = (split)[5];
    my $cds_start = (split)[6] eq "\\N" ? undef : split[6];
    my $cds_end = split[7] eq "\\N" ? undef : split[7];
    my $cdna_start = split[8] eq "\\N" ? undef : split[8];
    my $cdna_end = split[9] eq "\\N" ? undef : split[9];
    my $translation_start = split[10] eq "\\N" ? undef : split[10];
    my $translation_end = split[11] eq "\\N" ? undef : split[11];
    my $distance_to_transcript = split[12] eq "\\N" ? undef : split[12];
    my $display = split[13] eq "\\N" ? undef : split[13];
...
sub insert_mtmp_transcript_variation {
  my $dbhvar = shift;
  my $load_file = shift;
  ...
  while (<FH>) {
    ...
    my $feature_stable_id = (split)[1];
    my $allele_string = (split)[2];
    my $consequence_types = (split)[3];
    my $cds_start = (split)[4] eq "//N" ? undef : split[4];
    my $cds_end = split[5] eq "//N" ? undef : split[5];
    my $cdna_start = split[6] eq "//N" ? undef : split[6];
    my $cdna_end = split[7] eq "//N" ? undef : split[7];
    my $translation_start = split[8] eq "//N" ? undef : split[8];
    my $translation_end = split[9] eq "//N" ? undef : split[9];
    my $distance_to_transcript = split[10] eq "//N" ? undef : split[10
    ];
